...
class ShoppingListsRepository(private val db: ShoppingListsDatabase) {
    ...
    fun getItemsByCategoryId(categoryId: Long): Flow<List<ItemEntity>> {
        return db.itemCategoryDao().getCategoryWithItemsById(categoryId)
    }

    fun getItemsByPriorityId(priorityId: Long): Flow<List<ItemEntity>> {
        return db.itemDao().getPriorityWithItemsById(priorityId)
    }

    fun getItemsByPlaceToBuyId(placeToBuyId: Long): Flow<List<ItemEntity>> {
        return db.itemPlaceToBuyDao().getPlaceToBuyWithItemsById(placeToBuyId)
    }

    fun getItemsByListId(listId: Long): Flow<List<ItemEntity>> {
        return db.itemDao().getItemsByListId(listId)
    }

    fun getSelectedCategories(isDeletable: Boolean): Flow<List<ItemCategoryEntity>> {
        return db.itemCategoryDao().getSelectedCategories(isDeletable)
    }
    ...
    fun insertPlaceToBuy(placeToBuy: ItemPlaceToBuyEntity) =
        db.itemPlaceToBuyDao().insertPlaceToBuy(placeToBuy)
    ...
    fun deletePlaceToBuy(placeToBuy: ItemPlaceToBuyEntity) =
        db.itemPlaceToBuyDao().deletePlaceToBuy(placeToBuy)
    ...
    fun updatePlaceToBuy(placeToBuy: ItemPlaceToBuyEntity) =
        db.itemPlaceToBuyDao().updatePlaceToBuy(placeToBuy)
